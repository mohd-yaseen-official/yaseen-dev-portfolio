...
import { featuredClonesConfig } from "../../axiosConfig";

const FeaturedClones = () => {
    const [featuredClones, setFeaturedClones] = useState([]);
    const [loading, setLoading] = useState(true);
    const containerRef = useRef(null);
    const isInView = useInView(containerRef, { once: false, margin: "-50px" });

    useEffect(() => {
        featuredClonesConfig
            .get(
                ""
            )
            .then((response) => {
                setFeaturedClones(response.data.data);
                setLoading(false);
            })
            .catch((err) => {
    ...
    const renderProjectCards = () =>
        featuredClones.map((project, index) => (
            <motion.div
                key={project.id}
    ...
    return (
        ...
        >
            <meta itemProp="name" content="Featured Clones by Yaseen" />
            <meta
                itemProp="description"
                content="Collection of Yaseen's best coding UI clone projects"
            />
                ...
                >
                    ...
                    >
                        <Link
                            ...
                            aria-label="View all UI clones by Yaseen"
                            itemProp="url"
                        >
                            View All Clones
                        </Link>
